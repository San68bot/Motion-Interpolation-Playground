...
data class Vec3(var x: Double = 0.0, var y: Double = 0.0, var z: Double = 0.0) {
    ...
    operator fun times(other: Vec3): Double = (x * other.x + y * other.y + z * other.z)
}

object MathFunctions {
    fun dot(a: Vec3): Double = a * a
    ...
    fun isNormalizedEps(a: Vec3, epsilon: Double): Boolean = dot(a) >= (1.0 - epsilon) * (1.0 - epsilon) && dot(a) <= (1.0f + epsilon) * (1.0 + epsilon)
...
fun SingleBiarc(point: Vec3, tangent: Vec3, pointToMid: Vec3): SingleBiarcOutput {
    ...
    val denominator = 2.0 * (perpAxis * pointToMid)
    when {
    ...
    when {
        fracDist < arc1.biarc_arcLen -> {
            if (arc1.biarc_arcLen < epsilon) {
                pResult = arc1.biarc_center + arc1.biarc_axis1
            } else {
            ...
            if (arc2.biarc_arcLen < epsilon) {
                pResult = arc2.biarc_center + arc2.biarc_axis1
            } else {
...
fun BiarcPoints(p1: Vec3, t1: Vec3, p2: Vec3, t2: Vec3): ArrayList<BiarcInterpolationOutput> {
    val arc = ComputeBiarcs(p1, t1, p2, t2)
    println(arc.pArc1.biarc_arcLen)
    val points = arrayListOf<BiarcInterpolationOutput>()
    generateSequence(0) { it + 1 }
        .takeWhile { it <= 1000 }
        .forEach { points.add(arc interpolationAt it / 1000.0) }
    return points
}
...
fun main() {
    ...
    val points = BiarcPoints(p1, t1, p2, t2)
    println(points)
}